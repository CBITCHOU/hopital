        ...
        <div class="header-top wow fadeIn">
            <div class="container">
                <a class="navbar-brand" href="{{ url('/') }}"><img
                        src="{{ $app ? config('app.url') . 'storage/' . $app->logo_path :  config("app.url").'images/mayar.png' }}"
                        alt="">HOSPITAL_MANAGEMENT</a>
                    ...
                    <div class="header-info">
                        ...
                        <div class="info-inner">
                            ...
                            <span class="iconcont"><a data-scroll href="#">Daily:
                                    {{ $app ? $app->Working_Horse :'7:00am-8:00pm ' }}</a></span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <nav class="main-menu">
                    ...
                    <div id="navbar" class="navbar-collapse collapse">
                        <ul class="nav navbar-nav">
                            <li onclick="check_active('Home')"><a id="Home" href="{{ url('/') }}">Accueil</a></li>
                            ...
                            @auth
                                @if (auth()->user()->is_super_admin)
                                    <li onclick="check_active('admin-area')"><a id="admin-area" data-scroll
                                            href="{{ route('admin_settings') }}">administrateur</a></li>
                            ...
                            @endauth
                                        ...
                                        <form id="logout-form" method="POST" action="{{ route('logout') }}">
                                            @csrf
                                            <button id="logout-button" type="submit">Déconnexion</button>
                                        </form>
                                        @php
                                           if (session('logged_out')) {
                                               echo '<script>document.getElementById("logout-button").style.display = "none";</script>';
                                            }
                                        @endphp
